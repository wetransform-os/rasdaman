...
public class CrsProjectionUtil {
    ...
    public static GeoTransform getGeoTransformInTargetCRS(GeoTransform sourceGeoTransform, String targetCRS,
        ...
        String keyMap = StringUtil.join(sourceGeoTransform.getWKT(), targetCRS, sourceGeoTransform.toString(), targetCRS, targetCRSGeoXResolutionKey, targetCRSGeoYResolutionKey, targetCRSBBoxKey);
        ...
        // Here it uses gdal java warp() from gdal 2.x directly instead of writing to temp VRT file like in gdal 1.x
        Dataset srcDataset = gdal.Open(tempVRTFile.getAbsolutePath());
        Dataset[] srcDatasets  = new Dataset[] { srcDataset };
        Vector<String> vector= new Vector<>();
        vector.add("-t_srs");
        vector.add(targetCRSWKT);

        if (targetCRSGeoXResolution != null) {
            vector.add("-tap");
            vector.add("-tr");
            vector.add(targetCRSGeoXResolution.abs().toPlainString());
            vector.add(targetCRSGeoYResolution.abs().toPlainString());
        }

        if (targetCRSBBox != null) {
            vector.add("-te");
//            // e.g. xmin ymin xmax ymax
            vector.add(targetCRSBBox.getXMin().toPlainString());
            vector.add(targetCRSBBox.getYMin().toPlainString());
            vector.add(targetCRSBBox.getXMax().toPlainString());
            vector.add(targetCRSBBox.getYMax().toPlainString());
        }
        ...
        // Warp the VRT file to the memory
        try {
            Dataset targetDataset = gdal.Warp("/vsimem/" + StringUtil.addDateTimeSuffix("vrt_warped"), srcDatasets, warpOptions);
            result = new GeoTransform(targetDataset);
        } catch (Exception ex) {
            ...
            String optionsTmp = ListUtil.join(options, " ");
            String errorMessage = "Failed projecting source GeoTransform to target GeoTransform using gdal. Reason: " + ex.getMessage()
                                + ". Given source GeoTransform: " + new GeoTransform(srcDataset).toString() + " and gdal warp options: " + optionsTmp;
