 ...
 */
public class LegacyBbox implements Cloneable {

    private static Logger log = LoggerFactory.getLogger(LegacyBbox.class);
    ...
    private List<LegacyDomainElement> domains; // Cloning

    // spatial bbox
    private Double minX, maxX, minY, maxY, minZ, maxZ;

    public LegacyBbox(String crs, List<LegacyDomainElement> domains, String coverage)
    throws Exception {
        ...
        // Raw Bounding-box
        for (LegacyDomainElement el : domains) {
            if ((el.getMinValue() == null) || (el.getMaxValue() == null)) {
                throw new Exception("Invalid bounding box: null element encountered.");
            }
        ...
        if (crs == null) {
            throw new Exception("Invalid CRS name: null element encountered.");
        } else {
        ...
        for (LegacyDomainElement el : domains) {
            ...
            // X AXIS
            if (el.getType().equals(LegacyAxisTypes.X_AXIS)) {
                ...
                maxX = max;
                // Y AXIS
            } else if (el.getType().equals(LegacyAxisTypes.Y_AXIS)) {
                ...
                minY = min;
                maxY = max;
            } else if (el.getType().equals(LegacyAxisTypes.ELEV_AXIS)) {
                minZ = min;
                maxZ = max;
            }
        }
        ...
        if (!crsSourceX.isEmpty() && !crsSourceY.isEmpty()) {
            if (!CrsUtil.CrsUri.areEquivalent(crsSourceX, crsSourceY)) {
                throw new Exception("Invalid bounding box: X and Y axis have different CRS:" + crsSourceX + "<->" + crsSourceY);
            }
        }
    ...
    @Override
    public LegacyBbox clone() {
        try {
            return new LegacyBbox(crsName, domains, coverageName);
        } catch (Exception e) {
    ...
    public String getWgs84CrsName() throws PetascopeException {
        ...
        if (CrsUtil.CrsUri.isCompound(crsName)) {
            ...
            for (LegacyDomainElement dom : domains) {
                if (dom.getType().equals(LegacyAxisTypes.X_AXIS) ||
                        dom.getType().equals(LegacyAxisTypes.Y_AXIS)) {
    ...
    private int getSpatialDimensionality() {
        int counter = 0;
        for (LegacyDomainElement domain : domains) {
            if (domain.getType().equals(LegacyAxisTypes.X_AXIS)
                    || domain.getType().equals(LegacyAxisTypes.Y_AXIS)
                    || domain.getType().equals(LegacyAxisTypes.ELEV_AXIS)) {
                counter += 1;
            }
        }
        return counter;
    ...
    public String getLowerCorner() {
        ...
        for (int i = 0; i < getDimensionality(); i++) {
            ...
            //}
            output += LegacyBigDecimalUtil.stripDecimalZeros(tmp);
        }
        return output;
    }
    ...
    public String getWgs84LowerCorner() {
        ...
        for (int i = 0; i < getDimensionality(); i++) {
            ...
            if (getType(i).equals(LegacyAxisTypes.X_AXIS)) {
                output += getWgs84minLon();
            } else if (getType(i).equals(LegacyAxisTypes.Y_AXIS)) {
