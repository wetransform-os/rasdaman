...
# 1. Test doGet()
$CURL -H "Origin:*" -I "$PETASCOPE_URL" > "$OUTPUT_FILE"
...
$CURL -H "Origin: http://test.com"   -H "Access-Control-Request-Method: POST"   -H "Access-Control-Request-Headers: X-Requested-With" -X OPTIONS "$PETASCOPE_URL" --verbose 2> "$OUTPUT_FILE"
# Check the result to see Petascope allows CORS
...
$CURL -H "Origin:*" -I "$PETASCOPE_URL/GetCoverageExtents" > "$OUTPUT_FILE"
