...
WMS_TEST_END_POINT_TEMPLATE=$PETASCOPE_END_POINT"service=WMS&version=1.3.0&request=GetMap&layers=test_wms_4326&bbox=MIN_MAX_LAT_LONG_TEMPLATE&crs=EPSG:4326&width=600&height=600&styles=&format=image/png"
...
do
  ...
  numb_connection=$(psql -c "SELECT sum(numbackends) FROM pg_stat_database;" -d petascopedb 2>&1 | awk 'NR==3{print $1}')
