...
public class ReturnClauseHandler extends Handler {
    ...
    private VisitorResult handle(WcpsResult processingExpr) {
        ...
        if (metadata != null) {
            // e.g. max(c), here c is not set to null as before, because it is still used under VALUES expression of a coverage constructor to determine the grid bound
            // of an axis iterator
            if (!metadata.isChangedToNullByReductionExpression() && !metadata.getAxes().isEmpty()) {
