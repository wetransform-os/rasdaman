...
struct quiescent_state_based::thread_control_block : detail::thread_block_list<thread_control_block>::entry {
...
private:
  ...
  bool try_update_epoch(unsigned curr_epoch, unsigned new_epoch) {
    ...
    auto prevents_update = [old_epoch](const thread_control_block& data) {
      ...
      return data.local_epoch.load(memory_order) == old_epoch && data.is_active(memory_order);
    };

    // If any thread hasn't advanced to the current epoch, abort the attempt.
    bool cannot_update = std::any_of(global_thread_block_list.begin(), global_thread_block_list.end(), prevents_update);
    if (cannot_update) {
      return false;
    }
...
quiescent_state_based::guard_ptr<T, MarkedPtr>::guard_ptr(const MarkedPtr& p) noexcept : base(p) {
  if (this->ptr) {
    local_thread_data().enter_region();
  }
}
...
template <class T, class MarkedPtr>
quiescent_state_based::guard_ptr<T, MarkedPtr>::guard_ptr(const guard_ptr& p) noexcept : guard_ptr(MarkedPtr(p)) {}

template <class T, class MarkedPtr>
quiescent_state_based::guard_ptr<T, MarkedPtr>::guard_ptr(guard_ptr&& p) noexcept : base(p.ptr) {
  p.ptr.reset();
}
...
template <class T, class MarkedPtr>
bool quiescent_state_based::guard_ptr<T, MarkedPtr>::acquire_if_equal(const concurrent_ptr<T>& p,
